...
function init(){
    document.getElementById('todo-input').addEventListener("keypress",enterPressOnMainInput);
...
function getLocalStorage(){
    ...
    if(List && document.getElementById('todo-input')){
        List.forEach((element)=>{
            element.id=todoCount;
            enterPressOnMainInput({keyCode:13,target:{value:element.value}});
        });
        List.forEach((element)=>{
    ...
    if(localStorage.getItem('pageName') && document.getElementById('todo-input'))
    {
